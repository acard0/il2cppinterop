...
use crate::{il2cpp::FUNCTIONS, il2cpp_farproc};

use super::domain;

pub fn attach(domain: *mut c_void) -> *mut c_void {
    unsafe {
        let func = il2cpp_farproc!(fn(*mut c_void) -> *mut c_void, FUNCTIONS.m_thread_attach);
        func(domain)
    }
}
...
pub fn detach(thread: *mut c_void) {
    unsafe {
        let func = il2cpp_farproc!(fn(*mut c_void) -> *mut c_void, FUNCTIONS.m_thread_detach);
        func(thread);
    }
}
...
unsafe extern "system" fn handler(reserved: *mut c_void) -> u32 {
    unsafe {
        ...
        let on_start = il2cpp_farproc!(fn() -> *mut c_void, thread.on_start);

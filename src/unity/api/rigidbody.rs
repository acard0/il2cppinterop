...
impl CRigidbody {
    ...
    pub fn set_detect_collisions(&self, detect: bool) {
        unsafe {
            let func = il2cpp_farproc!(fn(*mut c_void, bool) -> *mut c_void, RIGIDBODY_FUNCTIONS.m_set_detect_collisions);
            func(self as *const _ as *mut _, detect);
        }
    }

    pub fn get_velocity(&self) -> Vector3 {
        let mut v_ret = Vector3 { x: 0.0, y: 0.0, z: 0.0 };
        unsafe {
            let func = il2cpp_farproc!(fn(*mut c_void, &mut Vector3) -> *mut c_void, RIGIDBODY_FUNCTIONS.m_get_velocity);
    ...
    pub fn set_velocity(&self, vector: Vector3) {
        unsafe {
            let func = il2cpp_farproc!(fn(*mut c_void, Vector3) -> *mut c_void, RIGIDBODY_FUNCTIONS.m_set_velocity);
